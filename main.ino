...
String outputLEDAliveState = "off";
String outputLaserState = "off";
int buttonState = 0;

// Assign output variables to GPIO pins
const int outputLEDAlive = 14;
...
void setup()
{
  ...
  pinMode(outputLEDAlive, OUTPUT);
  pinMode(outputLaser, OUTPUT);
  pinMode(LEDButton, OUTPUT);
  pinMode(inputButton, INPUT);
  // Set outputs to LOW
  digitalWrite(outputLEDAlive, LOW);
        ...
        { // if the byte is a newline character
          ...
          if (currentLine.length() == 0)
          {
            ...
            if (header.indexOf("GET /D5/on") >= 0)
            {
              ...
              outputLEDAliveState = "on";
              digitalWrite(outputLEDAlive, HIGH);
            }
            else if (header.indexOf("GET /D5/off") >= 0)
            {
              Serial.println("GPIO D5 off");
              outputLEDAliveState = "off";
              digitalWrite(outputLEDAlive, LOW);
            }
            ...
            Serial.println(digitalRead(inputButton));
            if (buttonState == 1)
            {
              client.println("<p>Button is click</p>");
            }
            else
            {
              client.println("<p>Button is not click</p>");
            }

            // Display current state, and ON/OFF buttons for GPIO 5
            client.println("<p>LED alive - State " + outputLEDAliveState + "</p>");
            // If the output5State is off, it displays the ON button
            if (outputLEDAliveState == "off")
            {
            ...
            client.println("<p>Laser - State " + outputLaserState + "</p>");
            // If the output4State is off, it displays the ON button
            if (outputLaserState == "off")
            {
